...
//   - API Key 出口也必须保留工具调用项上的 namespace。Codex 的 namespace 工具调用
//     需要把 response.output 原样回填到下一轮 input；中转层删除该字段会破坏该
//     回合链路。若某个兼容上游明确拒绝该字段，应由其专用兼容策略处理，不能在
//     OpenAI Responses 公共路径上静默丢失身份信息。
